...
## Learning Goals
- Set Database tables and associations using foreign keys
...
## App: Snacks on a Plane
  - snack -< user_snack >- user
  - snack: name (string), brand (string), flavor_profile (string)
  - user: name (string), img_url (string), motto (string)
  - user_snack: user_id (integer), snack_id (integer), rating (integer), review (string)

## Outline
- [✅] Add additional models to our domain
  - [✅] Set up macros using ActiveRecord
  - [✅]] Use information from the association in our views (show page for users)
  - [✅]] Make a create form for the new model with a dropdown (new user_snack form)
